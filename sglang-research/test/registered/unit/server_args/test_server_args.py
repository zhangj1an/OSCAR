...
class TestKVCacheQuantGroupSizeArgs(CustomTestCase):
    def test_prepare_server_args_parses_kv_cache_quant_group_size(self):
        server_args = prepare_server_args(
            [
                "--model-path",
                "dummy",
        ...
        self.assertEqual(server_args.kv_cache_dtype, "int4")
        self.assertEqual(server_args.kv_cache_quant_group_size, 64)

    def test_check_server_args_rejects_non_int_kv_dtype(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            "--kv-cache-quant-group-size is only supported",
        ):
            server_args.check_server_args()

    def test_check_server_args_rejects_non_positive_group_size(self):
        server_args = ServerArgs(
            model_path="dummy",
            served_model_name="dummy",
            kv_cache_dtype="int4",
            kv_cache_quant_group_size=0,
            chunked_prefill_size=-1,
        )
        with self.assertRaisesRegex(
            ValueError,
            "--kv-cache-quant-group-size must be positive",
        ):
            server_args.check_server_args()

    def test_check_server_args_rejects_hybrid_swa_models(self):
        server_args = ServerArgs(
            model_path="dummy",
            served_model_name="dummy",
            kv_cache_dtype="int4",
        ...
        server_args.model_path = "real-model"
        with patch.object(
            ServerArgs,
            "get_model_config",
            return_value=SimpleNamespace(is_hybrid_swa=True),
        ):
