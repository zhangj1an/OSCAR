...
class PrefillAdder:
    ...
    def _available_and_evictable(self) -> int:
    ...
    def add_chunked_req(self, req: Req):
        ...
        if self.dllm_config is not None:
            ...
            # A resumed chunked_req must keep ownership, but it must not run a
            # zero-token forward: overlap futures expect one output per request.
            if _rem_tokens <= 0:
