#!/usr/bin/env bash
# Compute K/V rotation checkpoints for GLM-4.7.
#
...
NUM_LAYERS="${NUM_LAYERS:-92}"   # GLM-4.7 has 92 layers
COMPOSITION="${COMPOSITION:-r_h_pbr}"
CHUNK_ID="${CHUNK_ID:-all}"

# Auto-find the latest calibration produced by save_qkv (sorts by mtime).
# Override with DUMP_PATH or CALIB_DIR for a specific calibration.
