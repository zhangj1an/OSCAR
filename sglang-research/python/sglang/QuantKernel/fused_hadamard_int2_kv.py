...
def validate_hadamard_order_for_kv_fuse_int2(hadamard_order: int, head_dim: int) -> None:
...
def _make_fused_kernel_int2(head_dim: int, hadamard_order: int):
    ...
    def _fused_hadamard_int2_set_kv_kernel(
        ...
        vals0 = tl.where(dim_mask, tl.gather(acc, safe_off0, 0), 0.0).to(tl.bfloat16).to(tl.float32)
        vals1 = tl.where(dim_mask, tl.gather(acc, safe_off1, 0), 0.0).to(tl.bfloat16).to(tl.float32)
        vals2 = tl.where(dim_mask, tl.gather(acc, safe_off2, 0), 0.0).to(tl.bfloat16).to(tl.float32)
        vals3 = tl.where(dim_mask, tl.gather(acc, safe_off3, 0), 0.0).to(tl.bfloat16).to(tl.float32)
