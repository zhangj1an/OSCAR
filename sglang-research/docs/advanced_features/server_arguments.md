...
- To enable Triton integer kv cache quantization, add `--kv-cache-dtype int2`, `--kv-cache-dtype int4`, or `--kv-cache-dtype int8`. Use `--kv-cache-quant-group-size` to split the head dimension into smaller quantization groups for better accuracy on the full-attention int KV path.
...
| `--kv-cache-dtype` | Data type for kv cache storage. "auto" will use model data type. "bf16" or "bfloat16" for BF16 KV cache. "fp8_e5m2" and "fp8_e4m3" are supported for CUDA 11.8+. "fp4_e2m1" (only mxfp4) is supported for CUDA 12.8+ and PyTorch 2.8.0+. "int2", "int4", and "int8" use the Triton quantized KV cache path. | `auto` | `auto`, `fp8_e5m2`, `fp8_e4m3`, `bf16`, `bfloat16`, `fp4_e2m1`, `int2`, `int4`, `int8` |
| `--kv-cache-quant-group-size` | Optional head-dimension group size for Triton integer KV cache quantization on the full-attention int KV path. If unset, one scale/zero pair is shared across the full K/V head. If set, the value must be positive and divide both `head_dim` and `v_head_dim`. This option is not supported with hybrid SWA KV cache layouts. | `None` | Type: Optional[int] |
