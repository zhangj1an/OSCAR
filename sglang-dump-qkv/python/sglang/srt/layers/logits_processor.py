...
# ─── CoQuant per-step next-token logit dump ──────────────────────────────────
import os as _os
_COQUANT_DUMP_LOGITS_ENABLED = _os.environ.get("SGLANG_DUMP_NEXT_LOGITS", "0") in ("1", "true", "True")
_COQUANT_DUMP_LOGITS_DIR = _os.environ.get("SGLANG_DUMP_LOGITS_DIR", "/tmp/coquant_logits")
_COQUANT_LOGIT_STEP_IDX = [0]


def _coquant_dump_next_logits(sampled_logits):
    """Save next-token logits to disk per call (only TP rank 0 — logits are post-allreduce)."""
    if not _COQUANT_DUMP_LOGITS_ENABLED or sampled_logits is None:
        return
    try:
        ...
        pass
    _os.makedirs(_COQUANT_DUMP_LOGITS_DIR, exist_ok=True)
    idx = _COQUANT_LOGIT_STEP_IDX[0]
    path = _os.path.join(_COQUANT_DUMP_LOGITS_DIR, f"logits_{idx:05d}.pt")
    torch.save(sampled_logits.detach().to(torch.float16).cpu().contiguous(), path)
    _COQUANT_LOGIT_STEP_IDX[0] += 1
...
class LogitsProcessor(nn.Module):
    ...
    def forward(
        ...
        if not logits_metadata.extend_return_logprob:
            ...
            # CoQuant: dump next-token logits before return (env-gated).
            _coquant_dump_next_logits(sampled_logits)
