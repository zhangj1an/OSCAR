...
SGLang also supports Triton-based integer KV cache quantization:

- **INT8**: Dynamic affine quantization with per-token metadata
- **INT4**: Packed 4-bit quantization with dynamic affine metadata
- **INT2**: Packed 2-bit quantization with dynamic affine metadata

For `int2`, `int4`, and `int8`, SGLang computes scale/zero pairs dynamically while writing the KV cache. By default, one scale/zero pair is shared across the full head dimension for each token and KV head.
...
# Enable INT4 KV cache (default: one scale/zero pair per head)
python3 -m sglang.launch_server \
    --model-path meta-llama/Llama-3.1-8B-Instruct \
    --kv-cache-dtype int4 \

# Enable INT4 KV cache with group-wise head-dim quantization
python3 -m sglang.launch_server \
    --model-path meta-llama/Llama-3.1-8B-Instruct \
    --kv-cache-dtype int4 \
    --kv-cache-quant-group-size 32 \
```

### Integer Group Size

`--kv-cache-quant-group-size` is available for the Triton integer KV cache path (`int2`, `int4`, `int8`) on full-attention models.

- If unset, SGLang keeps the legacy behavior and shares one scale/zero pair across the full K/V head.
- If set, SGLang splits the K and V head dimensions into contiguous groups of that size and stores one scale/zero pair per group.
- The group size must divide both `head_dim` and `v_head_dim`.
- This option is not supported with hybrid SWA KV cache layouts.

Smaller groups usually improve quantization accuracy, but they also increase scale-metadata overhead and may use slower fallback dequantization paths than the default per-head configuration.

### Hadamard rotation (int2 / int4)

- The int2 path always applies a segmented Walsh–Hadamard rotation to Q, K, and V before quantization. With the default (non-grouped) metadata layout, all three rotations are fused: Q inside the decode kernel, K/V inside the quantize-set kernel. When `--kv-cache-quant-group-size` is set, the kernel falls back to a non-fused path that rotates Q outside the decode kernel and runs the quantized set kernel on pre-rotated K/V.
- The int4 path applies the same rotation when Hadamard is enabled (`SGLANG_HADAMARD_ENABLE`). Group-wise int4 also disables the fused Q Hadamard and uses the pre-rotated K/V set path.
- int8 never rotates.

### Mixed-precision KV (`SGLANG_MIXED_KV_*`) and attention sinks

Mixed-precision KV windows (high-precision prefix + recent, quantized middle) on the Triton decode backend do not support attention sinks. Combining `SGLANG_MIXED_KV_*` with a sink-bearing model raises `NotImplementedError` up-front rather than silently ignoring the sink contribution.
